...
def nota_valida(nota):
    return bool(nota > 0 and nota <= 10)
...
""" Ordena la la lista de alumnos realizando una compracion entre hermanos, y poniendo en los primeros
lugares a los mejores promedios"""
...
""" muestra la informacion de todos los estudiante de la lista"""
...
def registrar_nota(lista_estudiantes):  
    ...
    while True:
        nueva_nota = input("Ingrese la nota obtenida, debe estar entre 0 y 10 : ")
        if(nota_valida(float(nueva_nota))): # asumimos que coloca un nro; sino hay que gestonar lso errores capturando bloque try ctach
            break

    lista_estudiantes[pos_estudiante]["calificaciones"].append(float(nueva_nota))
